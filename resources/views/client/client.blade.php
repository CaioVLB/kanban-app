<x-app-layout>
  <div class="py-12">
    <div x-data="client({{ json_encode($clients) }})" class="flex flex-wrap justify-start max-w-7xl mx-auto px-4 md:px-2">
        ...
        <div class="w-full flex justify-center items-center border border-gray-200 rounded-lg shadow bg-white p-4 mx-2 mb-2 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          ...
          <div class="w-full grid gap-4 sm:grid-cols-2 lg:grid-cols-4 py-1 px-4">
            ...
            <div class="flex flex-col col-span-1">
              ...
              <span class="text-start text-gray-900 font-bold truncate dark:text-white" x-text="client.cpf"></span>
            </div>
            <div class="flex flex-col col-span-1">
              <label class="text-start text-gray-600 dark:text-gray-400">Telefone</label>
              <span class="text-start text-gray-900 font-bold truncate dark:text-white" x-text="client.phone_number"></span>
            </div>
            <div class="flex flex-col col-span-1">
              <label class="text-start text-gray-600 dark:text-gray-400">E-mail</label>
              <span class="text-start text-gray-900 font-bold truncate dark:text-white" :title="client.email" x-text="client.email"></span>
            </div>
          </div>
          <a :href="'{{ route('client.show', '') }}' + '/' + client.id" class="flex items-center justify-center p-2 bg-amber-200 rounded-lg text-amber-600 border border-amber-300 hover:bg-amber-300 hover:text-amber-700 dark:bg-amber-600 dark:text-amber-200 dark:border-amber-700 dark:hover:text-white dark:hover:bg-amber-700">
      ...
      @include('client.snippets.client-modal', ['show' => true])
