<div x-data="client_evaluations()" class="flex-grow">
  @if(count($physiotherapy) > 0)
    <div class="flex items-center justify-end">
      <x-button-modal onclick="openModalEvaluation('create', {{ $client->id }}, 'physiotherapy', 'fisioterapêutica')" class="inline-flex items-center py-2 px-5 text-sm font-bold text-amber-600 focus:outline-none bg-amber-200 rounded-lg border border-amber-300 hover:bg-amber-300 hover:text-amber-700 dark:bg-amber-600 dark:text-amber-200 dark:border-amber-700 dark:hover:text-white dark:hover:bg-amber-700 cursor-pointer">
  ...
  <div class="flex flex-col max-h-[380px] max-h-[380px]:overflow-scroll scroll-smooth overflow-x-hidden gap-y-1.5 pe-2 mt-3">
    @forelse($physiotherapy as $physio)
      <div class="flex justify-center items-center border border-gray-200 rounded-lg shadow bg-white p-2 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="w-full grid grid-cols-6 gap-x-4 gap-y-1 lg:py-1 px-2">
          <div class="flex flex-col md:col-span-4 col-span-full">
            ...
            <span class="text-start text-sm text-gray-900 font-bold truncate dark:text-white" title="{{ $physio->evaluation_name }}">{{ $physio->evaluation_name }}</span>
          </div>
          <div class="flex flex-col md:col-span-2 col-span-full relative group">
            ...
            <div class="absolute top-0 left-1/3 transform -translate-x-1/2 w-60 p-2 bg-gray-600 text-white text-xs rounded shadow-lg hidden group-hover:block z-50 cursor-default">
              Avaliação realizada na data <b>{{ $physio->date }}</b>.
              @if($physio->collaborator)
                Profissional responsável: <b>{{ $physio->collaborator->user->name }}</b>
              @endif
            </div>
          </div>
        </div>
        <div class="flex md:flex-row flex-col gap-2">
          <a href="{{ route('client.evaluation.evolutions', ['client_id' => $client->id, 'type' => $physio->type, 'evaluation_id' => $physio->id]) }}" title="Evolução do paciente"
          ...
          <x-button-modal onclick="openModalEvaluation('edit', {{ $client->id }}, '{{ $physio->type }}', 'fisioterapêutica', {{ $physio->id }}, '{{ $physio->evaluation_name }}')" class="flex items-center justify-center p-2 bg-amber-200 rounded-lg text-amber-600 border border-amber-400 hover:bg-amber-300 hover:text-amber-700 dark:bg-amber-600 dark:text-amber-200 dark:border-amber-700 dark:hover:text-white dark:hover:bg-amber-700">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
              <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
            </svg>
          </x-button-modal>
          <x-danger-button onclick="openModalDeleteEvaluation({{ $physio->id }}, '{{ $physio->type }}', '{{ $physio->evaluation_name }}')" :title="'Excluir avaliação'" class="flex items-center justify-center p-2 bg-red-200 rounded-lg border border-red-400 shadow hover:bg-red-300 dark:bg-red-400 dark:border-red-500">
    ...
    @empty
      <div class="w-full">
        ...
        <div class="p-3 mx-0 mt-2">
          <p class="py-1 px-1 mb-6 text-center text-gray-600 dark:text-gray-100">Aqui você poderá ver todas as avaliações fisioterapêutica deste paciente.</p>
          <div class="flex justify-center items-center">
            <x-button-modal onclick="openModalEvaluation('create', {{ $client->id }}, 'physiotherapy', 'fisioterapêutica')" class="inline-flex items-center py-2.5 px-5 text-sm font-bold text-amber-600 focus:outline-none bg-amber-200 rounded-lg border border-amber-300 hover:bg-amber-300 hover:text-amber-700 dark:bg-amber-600 dark:text-amber-200 dark:border-amber-700 dark:hover:text-white dark:hover:bg-amber-700 cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                   stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
                   class="w-5 h-5 me-1 lucide lucide-notepad-text">
                <path d="M8 2v4"/><path d="M12 2v4"/><path d="M16 2v4"/><rect width="16" height="18" x="4" y="4" rx="2"/><path d="M8 10h6"/><path d="M8 14h8"/><path d="M8 18h5"/>
