<div class="w-full bg-white dark:bg-gray-800 shadow rounded-lg p-4 flex flex-col items-start">
  ...
  <div x-data="client_notes()" class="mt-2 w-full">
    <form action="{{ route('client.storeNotes', $client->id) }}" method="POST" @submit.prevent="isLoadingNoteAdd = true; $el.submit()">
      @csrf
      <div class="flex">
        ...
        <button type="submit"
                x-bind:disabled="isLoadingNoteAdd"
          ...
          <span x-show="isLoadingNoteAdd" x-transition>
    ...
    <div class="flex flex-col max-h-[460px] max-h-[460px]:overflow-scroll scroll-smooth overflow-x-hidden gap-y-0.5 mt-2 p-1">
      @forelse($notes as $note)
        <div class="grid grid-cols-6 w-full h-auto max-h-[90px] box-border p-1.5 mt-1.5 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          ...
          <div class="col-span-1 flex items-center justify-center">
            <x-danger-button onclick="openModalDeleteNote({{ $note->id }}, '{{ $note->created_at }}')" :title="'Excluir anotação'" class="flex items-center justify-center p-2 bg-red-200 rounded-lg border border-red-400 shadow hover:bg-red-300 dark:bg-red-400 dark:border-red-500">
      ...
      @empty
        <div class="border rounded-lg shadow bg-yellow-100 p-3 mt-1 dark:border-yellow-800 dark:bg-yellow-600">
          <p class="p-1 text-start text-yellow-700 dark:text-yellow-100">Até o momento, nenhuma anotação foi registrada para este cliente.</p>
        </div>
      @endforelse
    </div>
    @include('client.snippets.dashboard.client-notes.delete-note-modal')
  </div>
</div>
