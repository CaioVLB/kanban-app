<div x-data="collaborator_addresses()" class="flex-grow">
  ...
  <div class="flex flex-col max-h-[380px] max-h-[380px]:overflow-scroll scroll-smooth overflow-x-hidden gap-y-1.5 pe-2 mt-6">
    @forelse($addresses as $address)
      <div class="flex justify-center items-center border border-gray-200 rounded-lg shadow bg-white p-2 gap-2 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="w-full grid lg:grid-cols-7 md:grid-cols-4 grid-cols-1 gap-x-4 gap-y-1 lg:py-1 px-2">
          ...
          <div class="flex flex-col lg:col-span-3 col-span-full">
            <label class="text-start text-gray-600 dark:text-gray-400">Endereço</label>
            <span class="text-start text-gray-900 font-bold md:truncate dark:text-white"
    ...
    @empty
      <div class="border rounded-lg shadow bg-yellow-100 p-3 mx-2 mt-2 dark:border-yellow-800 dark:bg-yellow-600">
        <p class="py-1 px-1 text-center text-yellow-700 dark:text-yellow-100">Até o momento, nenhum endereço foi cadastrado.</p>
      </div>
    @endforelse
  </div>
  @include('collaborator.snippets.dashboard.collaborator-categories.delete-address-modal', ['show' => true])
</div>
