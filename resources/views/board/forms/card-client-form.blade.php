<div class="w-full md:w-1/2 relative md:pl-2">
  <label for="card-priority" class="block text-sm font-medium text-gray-900 dark:text-white">Atribuir a um Cliente</label>
  <div class="mt-1 relative" @click.outside="openClientDropdown = false">
    <button @click="openClientDropdown = !openClientDropdown" type="button"
      ...
      <span
        :class="{'text-gray-700 || dark:text-white': selectedClientDropdown !== '', 'text-gray-500 || dark:text-gray-400': selectedClientDropdown === ''}"
        class="block truncate" x-text="selectedClientDropdown || 'Prioridade do cartão'">
      </span>
      <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
        <svg :class="{'transform rotate-180': openClientDropdown, 'transform rotate-0': !openClientDropdown}"
    ...
    <div x-show="openClientDropdown"
      ...
      <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3"
        ...
        <template x-for="priority in priorities" :key="priority.id">
          <li @click="selectedOption('client', priority)"
