<x-app-layout>
    <div class="py-12">
        <div x-data="{ openModal: false, team: [] }" class="flex flex-wrap justify-start max-w-7xl mx-auto px-4 md:px-2">
            <div class="w-full flex justify-between items-center mb-4 px-2">
                <h1 class="font-bold text-gray-500 dark:text-white">Gerenciador de Equipes</h1>
                <x-button-modal modal_id="create-team">
                    <svg class="me-1 -ms-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                    Criar Nova Equipe
                </x-button-modal>
            </div>
            <template x-if="team.length === 1">
                <div class="w-full flex justify-center items-center border border-gray-200 rounded-lg shadow bg-white p-4 mx-2 mb-2 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    ...
                    <div class="w-full grid grid-cols-3 gap-4 sm:grid-cols-2 lg:grid-cols-4 py-2 px-4">
                        <div class="flex flex-col">
                            <label class="text-start text-gray-600">Nome da equipe</label>
                            <span class="text-start text-gray-900 font-bold dark:text-white">
                                Exclusive bee team
                            </span>
                        </div>
                        <div class="flex flex-col">
                            <label class="text-start text-gray-600">Membros</label>
                            <span class="text-start text-gray-900 font-bold dark:text-white">
                                Caio Vitor e Anderson Krautheim
                            </span>
                        </div>
                    </div>
            ...
            <template x-if="team.length === 0">
                <div class="w-full flex justify-center items-center border rounded-lg shadow bg-yellow-100 p-4 mx-2 dark:border-yellow-800 dark:bg-yellow-600">
                    ...
                    <span class="py-2 px-4 text-start text-yellow-700 dark:text-yellow-100">
                        Até o momento, você não tem nenhuma equipe cadastrada. Utilize o botão acima para criar equipes.
                    </span>
                </div>
            </template>
        </div>
    ...
    <template x-if="true">
		@include('team.snippets.team-modal')
	</template>
